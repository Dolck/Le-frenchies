...
value (Num n) _ = n
value (Var v) d = fromMaybe (error $ "Variable " ++ v ++ " not defined") $ Dictionary.lookup v d
value (Add t u) d = value t d + value u d
value (Sub t u) d = value t d - value u d
value (Mul t u) d = value t d * value u d
value (Div t u) d 
  | rval == 0 = error "Divizion by zero"
  | otherwise = value t d `div` rval
    where rval = value u d
