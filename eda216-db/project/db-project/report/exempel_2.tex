...
In our system we don't have access to a barcode scanner, therefore a pallet is
created on button clicks in the user-interface. The pallets page have a filtering 
feature implemented, where filtering can be based on production time, cookie type, 
pallet status, and pallet id.
...
updated accordingly. The database supports all of the other requirements, 
though it is not part of the production interface.
...
The database supports all of these requirements, 
though it is not part of the production interface.
The database contains recipes which is used to update the raw materials upon 
pallet production.
...
model-view-controller pattern. The view section is mostly 
...
and closes the connection. When an error occurs during a transaction, 
a rollback is executed to preserve the state of the database.
...
There are no other functional dependencies except for the key dependencies, so 
...
have them installed, install them now. Inside your unzipped folder there is a 
folder named bin. Go there. To run the program type the following:
\paragraph{Unix:}
(in the bin folder)
\begin{lstlisting}[frame=single]  % Start your code-block
...
This will output a port in the terminal that you can later used to use the 
