...
The assignment brought to us was to build a program for a cookie factory. The client was Krusty Cookies. They needed a program to help them manage their production, deliveries and orders. The program needs therefore to both contain a database-system and a user interface for Krusty Cookies. Throughout the project we faced different challenges setting up a local-server, planning effective relations, managing the database, designing a user-friendly webpage. By using new framework and developing tools the results were remarkable.
